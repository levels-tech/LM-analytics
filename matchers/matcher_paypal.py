...
class PaypalMatcher(PaymentMatcher):
    ...
    def match(self, mese, anno):
        ...
        expected_date = f"{str(mese).zfill(2)}/{anno}"
        df_na = df_full[df_full["Data"].isna()]

        df_filtered = df_full[df_full["Data"].str[3:] == expected_date].copy()

        if len(df_filtered) == 0:
            found_dates = sorted(df_full["Data"].str[3:].unique())
            raise DateMismatchError(
                message=f"Nessun pagamento con paypal trovato per il periodo {expected_date}",
